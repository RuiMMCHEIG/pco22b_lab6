...
void WindowInterface::set_link(int from, int to){
    emit sig_set_link(from, to);
}

void WindowInterface::initialize(unsigned int nbMines, unsigned int nbFactories, unsigned int nbWholesalers) {
    if(sm_didInitialize){
    ...
    mainwindow = new MainWindow(nbMines, nbFactories, nbWholesalers, nullptr);
    mainwindow->show();
